CREATE TABLE Job(jobId INTEGER PRIMARY KEY NOT NULL AUTO_INCREMENT, jobTitle VARCHAR(50) not null, industry VARCHAR(25) not null, description VARCHAR(100) not null, companyId INTEGER not null, FOREIGN KEY companyId(companyId) REFERENCES Company(companyId), managerId INTEGER not null, FOREIGN KEY managerId(managerId) REFERENCES Manager(managerId), isInternship BOOL not null);
