...
CREATE TABLE Job(jobId INTEGER PRIMARY KEY NOT NULL AUTO_INCREMENT, jobTitle VARCHAR(50) not null, industry VARCHAR(25) not null, description VARCHAR(100) not null, companyId INTEGER not null, FOREIGN KEY (companyId) REFERENCES Company(companyId), isInternship BOOL not null);
