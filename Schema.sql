...
CREATE TABLE Manager(managerId INTEGER PRIMARY KEY NOT NULL AUTO_INCREMENT, companyId INTEGER not null, FOREIGN KEY companyId(companyId) REFERENCES Company(companyId), name VARCHAR(50) not null, technicalExperience BOOL not null, yearsAtCompany INTEGER not null);
